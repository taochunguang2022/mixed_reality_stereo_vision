...
for ii = 1:length(x) 
    plot3(y(ii), x(ii), ELECTRODE_HEIGHT, EMARKER, 'Color', ECOLOR, 'markersize', EMARKERSIZE, 'linewidth', EMARKERLINEWIDTH);
    
    text(y(ii), x(ii), ELECTRODE_HEIGHT, loc_file(ii).labels, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontWeight', 'bold');%loc_file(ii).labels 是电极的名称，HorizontalAlignment 和 VerticalAlignment 属性用于确保文本标签相对于电极标记的位置正确对齐，FontWeight 设置为 'bold' 使文本更加突出。
end
